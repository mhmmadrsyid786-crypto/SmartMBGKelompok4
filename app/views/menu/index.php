                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="#" onclick="openUpdateModal('<?= esc($m['id_menu']); ?>', '<?= esc($m['nama_menu']); ?>', '<?= esc($m['jenjang']); ?>', '<?= esc($m['kategori']); ?>', '<?= esc($m['kalori']); ?>', '<?= esc($m['protein']); ?>', '<?= esc($m['deskripsi']); ?>'); return false;" class="btn-sm btn-sm-edit">Edit</a>
...
<input type="hidden" name="csrf_token" value="<?= isset($_SESSION['csrf_token']) ? $_SESSION['csrf_token'] : '' ?>">
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <select name="kategori" required style="width:100%; padding:10px; border-radius:8px; border:1px solid var(--glass-border); background:var(--glass-bg); color:var(--text-primary);">
...
<input type="hidden" name="csrf_token" value="<?= isset($_SESSION['csrf_token']) ? $_SESSION['csrf_token'] : '' ?>">
                    ...
                    <div style="margin-bottom: 15px;">
                        <label style="display:block; margin-bottom:5px; color:var(--text-secondary);">Kategori</label>
                        <select name="kategori" id="edit_kategori" required style="width:100%; padding:10px; border-radius:8px; border:1px solid var(--glass-border); background:var(--glass-bg); color:var(--text-primary);">
                            <option value="Makanan Utama">Makanan Utama</option>
                            <option value="Lauk">Lauk</option>
                            <option value="Sayur">Sayur</option>
                            <option value="Buah">Buah</option>
