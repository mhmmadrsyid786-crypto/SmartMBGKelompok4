        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="#" onclick="openUpdateModal('<?= esc($s['id_sekolah']); ?>', '<?= esc($s['npsn']); ?>', '<?= esc($s['nama_sekolah']); ?>', '<?= esc($s['jenjang']); ?>', '<?= esc($s['jumlah_siswa']); ?>', '<?= esc($s['status_sekolah']); ?>', '<?= esc($s['alamat_sekolah']); ?>', '<?= esc($s['telepon']); ?>', '<?= esc($s['nama_kepsek']); ?>'); return false;" class="btn-sm btn-sm-edit">Edit</a>
...
<input type="hidden" name="csrf_token" value="<?= isset($_SESSION['csrf_token']) ? $_SESSION['csrf_token'] : '' ?>">
            ...
            <div style="margin-bottom: 15px;">
                ...
                <select name="jenjang" style="width:100%; padding:10px; border-radius:8px; border:1px solid var(--glass-border); background:var(--glass-bg); color:var(--text-primary);">
...
<input type="hidden" name="csrf_token" value="<?= isset($_SESSION['csrf_token']) ? $_SESSION['csrf_token'] : '' ?>">
            ...
            <div style="margin-bottom: 15px;">
                <label style="display:block; margin-bottom:5px; color:var(--text-secondary);">Jenjang</label>
                <select name="jenjang" id="edit_jenjang" style="width:100%; padding:10px; border-radius:8px; border:1px solid var(--glass-border); background:var(--glass-bg); color:var(--text-primary);">
                    <option value="TK">TK</option>
                    <option value="SD">SD</option>
                    <option value="SMP">SMP</option>
                    <option value="SMA">SMA</option>
