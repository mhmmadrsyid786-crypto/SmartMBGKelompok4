    ...
    <main class="main-content">
        <header class="topbar">
            ...
            <div class="user-profile">
                ...
                <?php
                $nama_user = isset($_SESSION['nama']) ? strtolower(trim($_SESSION['nama'])) : '';
                $avatar_img = '';
                $custom_foto = null;

                if (isset($_SESSION['user_id'])) {
                    // Fetch latest foto_profil directly from DB to reflect updates instantly
                    $db_header = new Database();
                    $db_header->query('SELECT foto_profil FROM users WHERE id_user = :id');
                    $db_header->bind('id', $_SESSION['user_id']);
                    $user_header = $db_header->single();
                    if ($user_header && !empty($user_header['foto_profil'])) {
                        $custom_foto = $user_header['foto_profil'];
                        $_SESSION['foto_profil'] = $custom_foto; // Update session
                    }
                }

                if ($custom_foto) {
                    // Use custom uploaded photo
                    echo '<div class="avatar" style="background-image: url(\'' . BASEURL . '/img/profil/' . esc($custom_foto) . '\'); background-size: cover; background-position: center; color: transparent; border: 2px solid white;"></div>';
                } else {
                    // Fallback to pravatar
                    if (strpos($nama_user, 'rasyid') !== false) $avatar_img = '11';
                    elseif (strpos($nama_user, 'aldi') !== false) $avatar_img = '12';
                    elseif (strpos($nama_user, 'suhabil') !== false) $avatar_img = '33';
                    elseif (strpos($nama_user, 'fauzan') !== false) $avatar_img = '14';
                    elseif (strpos($nama_user, 'shaqilla') !== false) $avatar_img = '5';
                    elseif (strpos($nama_user, 'dewi') !== false) $avatar_img = '9';
                    
                    if ($avatar_img) {
                        echo '<div class="avatar" style="background-image: url(\'https://i.pravatar.cc/150?img=' . $avatar_img . '\'); background-size: cover; color: transparent; border: 2px solid white;"></div>';
                    } else {
                        $inisial = isset($_SESSION['nama']) ? strtoupper(substr($_SESSION['nama'], 0, 1)) : 'U';
                        echo '<div class="avatar">' . $inisial . '</div>';
                    }
                }
                ?>
            </div>
